<template>
	...
	<view class="cu-bar bg-white search" :style="[{top:CustomBar + 'px'}]">
        <view class="search-form round">
				<input v-model="searchForm.yimiaoName" type="text" placeholder="疫苗"></input>
		</view>
...
<view class="list" :style='{"borderRadius":0,"backgroundColor":"rgba(255, 255, 255, 0)"}'>
    <view style="margin: 10rpx 10rpx 10rpx 10rpx;"
            ...
            :style='{"borderRadius":"40rpx","backgroundColor":"#008761"}'
        ...
        <image :style='{"borderRadius":"40rpx","width":"50%","height":"200rpx"}' mode="aspectFill" class="listmpic"
               :src="product.yimiaoPhoto?product.yimiaoPhoto.split(',')[0]:''"></image>
        <view class="listmr">
           ...
           <view style="display: flex;justify-content: space-between;float: left;margin-top: 20%;">
            <text style="margin-left: 50rpx;" v-if="isAuth('yimiaoCollection','修改')" class="cuIcon-edit" @click.stop="onUpdateTap(product.id)">
                修改
            </text>
            <text v-if="isAuth('yimiaoCollection','删除')" class="cuIcon-delete"
            ...
            v-if="isAuth('yimiaoCollection','新增')" class="add-btn" @click="onAddTap()">新增</button>
	...
	export default {
		...
		async onShow() {
            ...
            _this.btnColor = _this.btnColor.sort(() => {
	   		...
	   		//当前表的 类型 字段 字典表查询方法
		  	let yimiaoCollectionTypes = await this.$api.page('dictionary', {
			 	 page: 1,
			  	limit: 100,
		  		dicCode:'yimiao_collection_types'
		  	});
		  	this.yimiaoCollectionTypesList = yimiaoCollectionTypes.data.list;
		...
		methods: {
			//查询条件切换
			queryChange(e) {
				this.searchForm.yimiao_collectionName = "";
			},
			//类别搜索
			...
			async upCallback(mescroll) {
                ...
                let res = await this.$api.page(`yimiaoCollection`, params);
			...
			onDeleteTap(id) {
				...
				uni.showModal({
					...
					success: async function(res) {
						if (res.confirm) {
							await _this.$api.del('yimiaoCollection', JSON.stringify([id]));
							_this.hasNext = true
							// 重置分页参数页数为1
			...
			async search() {
                ...
                let res = await this.$api.page(`yimiaoCollection`, searchForm);
				// 如果是第一页数据置空
