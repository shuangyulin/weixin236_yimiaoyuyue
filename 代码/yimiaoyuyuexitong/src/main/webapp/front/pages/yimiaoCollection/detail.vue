   	 ...
   	 <view style="margin-top: 10rpx;"></view>
					...
					<view class="detail-list-item"
						...
						<view class="lable"
							...
							类型：</view>
						<view class="text" :style='{"color":"#008761","textAlign":"center","fontSize":"24rpx"}'>
                            {{detail.yimiaoCollectionValue}}</view>
					</view>
...
<script>
	export default {
		...
		methods: {
			...
			// 获取详情
			async init() {
				let res = await this.$api.info('yimiaoCollection', this.id);
				this.detail = res.data;
				// 主图
				this.swiperList = this.detail.yimiaoCollectionPhoto ? this.detail.yimiaoCollectionPhoto.split(",") : [];
			},
